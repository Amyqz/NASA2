...
#eta*(1-exp(-dose[160]/d0))^X
# dE = function(t, E, parms) {# 
#   list(eta*X*(1-exp(-t/d0)+eps)^(X-1))
# }
# yini = 0
# by= 1; dose = .0001*seq(0, 600, by =by)#RKS added step size; just for graphing or zeroing in
# outt = ode(yini, dose, dE, parms)
# plot(outt, xlab = "Dose (Gy)", type = "l", ylab = "Effect")#RKS
# yy=eta*X*(1-exp(-dose/d0)+eps)^(X-1)
# plot(dose[-1],yy[-1])
